...
import { spawnContainer, destroyContainer } from "../container.js";
...
appsRouter.get("/", async (c) => {
  ...
  const response: AppListResponse = { owned: rows.map(rowToApp), collaborations: [] };
...
appsRouter.post("/", async (c) => {
  ...
  const [record] = await db.insert(apps).values({
  ...
  spawnContainer(record.id).catch(async (err) => {
    console.error(`[orchestrator] Failed to start container for app ${record.id}:`, err);
